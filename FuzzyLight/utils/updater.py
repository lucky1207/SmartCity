...
class Updater:
    ...
    def load_sample_with_forget(self, i):
        ...
        try:
            ...
            # 每次都需要更新 sample index，因为它们的sample长度不同
    ...
    def update_network(self, i):
        ...
        if "IS_CONTINOUS" in self.dic_traffic_env_conf.keys():
            name = "/round_{0}_inter_{1}".format(self.cnt_round, self.agents[i].intersection_id)
            self.agents[i].save_network(self.dic_path["PATH_TO_MODEL"] + name)
        else:
